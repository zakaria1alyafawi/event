...
class AddUserRoute(BaseRoute):
    ...
    def add_user(self):
        ...
        try:
            ...
            required_fields = ["first_name", "last_name", "email", "password", "job_title"]
            missing = [field for field in required_fields if field not in data]
            if missing:
                return error_response(f"Missing required fields: {', '.join(missing)}", 400)

            # Normalize job_title for comparison and lookup
            job_title_raw = data["job_title"]
            job_title_normalized = job_title_raw.strip().lower()

            # ─── Authorization decision ─────────────────────────────────────
            requires_auth = job_title_normalized != "visitor"
            ...
            if requires_auth:
                ...
                if not auth_header:
                    return error_response(
                        "Authorization header required for non-visitor roles", 401
                    )
                ...
                user_id = auth_result  # admin/manager who is creating the user
            ...
            new_user = adder.add(
                ...
                phone=phone,
                job_title=job_title_raw,           # store original casing
                photo_url=photo_url,
                country=country,
                city=city,
            ...
            role_type = retrieve_user_types.get_by_name(job_title_normalized)

            if not role_type:
                return error_response(
                    f"Invalid job_title: '{job_title_raw}' (no matching role found)",
                    400
                )
            ...
            user_data = {
                ...
                "roles": [{
                    "name": new_user.job_title,  # or role_type.name if you prefer DB value
            ...
            logger.info(
                ...
                f"job_title: '{job_title_raw}' (normalized: '{job_title_normalized}') | "
                f"event_id: {event_id} | "
                f"by: {user_id or 'self-registration'}"
            )
