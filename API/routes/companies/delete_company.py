...
from Models.Companies.DeleteCompanies import DeleteCompanies
from Models.Utility import validate_uuid
import logging

logger = logging.getLogger("api.routes.companies.delete_company")

class DeleteCompanyRoute(BaseRoute):
    def __init__(self):
        super().__init__()
        self.auth_manager = AuthManager()
        logger.info("DeleteCompanyRoute initialized")

    def register_routes(self):
        self.bp.route("/delete_company", methods=["POST"])(self.delete_company)
    ...
    def delete_company(self):
        ...
        try:
            ...
            delete_companies = DeleteCompanies(session)
            success = delete_companies.delete(id)

            if success:
                logger.info(f"Deleted company {id} by user {user_id}")
                return success_response({"message": "Company deleted successfully"})
            else:
                logger.warning(f"Company {id} not found for deletion")
                return error_response("Company not found", 404)

        except ValueError as ve:
            logger.warning(f"Validation error in delete_company: {ve}")
